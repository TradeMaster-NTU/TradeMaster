...
parser.add_argument(
    "--dict_trained_model",
    default="result/SOTA/trained_model/",
...
parser.add_argument(
    "--result_dict",
    type=str,
    default="result/SOTA/test_result/",
...
def env_creator(env_name):
    if env_name == 'portfolio':
        from env.PM.portfolio_management import TradingEnv as env
    else:
...
class trader:
    def __init__(self, args):
        ...
        # self.model_config = a2c.A2C_DEFAULT_CONFIG.copy()
        ...
        register_env(self.env_name, lambda config: env_creator(self.env_name)
                     (config))
        self.model_config["env"] = TradingEnv
    ...
    def train_with_valid(self):
        ...
        trainer = select_algorithms(self.name_of_algorithms)
        print(trainer)
        self.trainer = trainer(env=env, config=config)
