...
parser.add_argument("--state_length",
                    type=int,
                    default=10,
                    help="the length of the dataframe our state contains, which will be used as the input of our net work, whose input is 2 dimension")
parser.add_argument("--target_order",
                    type=int,
                    default=1,
...
class TradingEnv(gym.Env):
    ...
    def step(self,action:np.array):
        ...
        if free_cash<0 or free_bitcoin<0:
            raise Exception("Something is wrong witht the order you place and there is no enough free cash or bitcoin in our portfolios, \
        ...
        self.public_state=self.data[self.tech_indicator_list].values.tolist()
        left_order=self.target_order-(self.portfolio[2]+self.portfolio[3])
        ...
        if self.terminal:
            ...
            for index in self.df.index:
                cash_used=cash_used+single_volume*self.data["midpoint"]*(1+self.data["asks_distance_0"]*0.01)
            ...
            if left_order>0:
                ...
                cash_left=cash_left-left_order*self.data["midpoint"]*(1+self.data["asks_distance_0"]*0.01)
            if left_order<0:
                cash_left=cash_left-left_order*self.data["midpoint"]*(1+self.data["bids_distance_0"]*0.01)
            print(cash_left)
