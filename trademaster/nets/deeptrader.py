...
class GCN(nn.Module):
    ...
    def forward(self, A, H_l):
        ...
        A = (A.T / sum).T
        A = torch.from_numpy(A).to(torch.float32).to(H_l.device)
...
class AssetScoringNet(Net):
    def __init__(self,
                 ...
                 kernel_size=2,
                 dropout=0.2) -> None:
        super(AssetScoringNet, self).__init__()
...
@NETS.register_module()
class AssetScoringValueNet(Net):
    def __init__(self,
                 N,
                 K_l,
                 num_inputs,
                 ...
                 kernel_size=2,
                 dropout=0.2) -> None:
        super(AssetScoringValueNet, self).__init__()
        self.TCN = TemporalConvNet(num_inputs, num_channels)
        self.SA = SA(num_channels[-1], N, K_l)
        self.GCN = GCN(K_l)
...
@NETS.register_module()
class MarketScoringNet(nn.Module):
    def __init__(self, n_features, hidden_size=12):
        super(MarketScoringNet, self).__init__()
