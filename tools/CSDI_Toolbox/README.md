...
The default data format requires six indicators: date, open, high, low, close, adjcp. Please make changes of line 9, 33 and 34 in dataset_own.py if you need to add, drop or change indicators.  

Step 1: Training and imputation of own dataset
   ```
   python tools/CSDI_Toolbox/exe_own.py --dataset [dataset name] --tic [ticker name]  --testmissingratio [missing ratio]
   ```
...
Step 2: Generation of new dataset
   ```
   python tools/CSDI_Toolbox/impute.py --dataset [dataset name] --tic [ticker name]  --testmissingratio [missing ratio]
   ```


Step 3: Visualization
   ```
   python tools/CSDI_Toolbox/visual.py --dataset [dataset name] --tic [ticker name]  --testmissingratio [missing ratio] --dataind [data index]
   ```
to visualize point chart of open, high, low, close and adjcp of 10-days interval as well as the candlestick chart

visualize_example.ipynb is a notebook for directly visualizing results.
